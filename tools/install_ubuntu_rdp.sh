...
if [[ "$version" = *"Ubuntu 17.10"* ]] || [[ "$version" = *"Ubuntu 18.04"* ]] || [[ "$version" = *"Ubuntu 20.04"* ]];
then
	echo
...
sudo apt-get install xrdp xorg xserver-xorg-core xorgxrdp -y
...
sudo apt-get install gnome-tweak-tool adwaita-icon-theme-full -y
